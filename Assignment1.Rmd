...
Answer : the 2nd statement is false.
...
```
```{r samp1-summ}
samp1 %>% summarize(samp_mean = mean(area), 
...
<li> Sample size of 1000. </li>
 ...
 Answer : sample size of 1000 will have most accurate mean
...
```{r sampling-dist}
sample_means50 %>% summarize(count = n())
...
sample_mean_small <- ames %>%
  ...
  summarize(samp_mean = mean(area))

ggplot(sample_mean_small, aes(x= samp_mean)) +geom_histogram(binwidth = 20)
```
...
sample_mean_small %>% summarize(count = n())
...
Answer: 3. Each element is representation of true population mean.
...
```{r price-sample}
ames %>% sample_n(size = 50) %>% summarize(samp_mean = mean(price))
...
sample_mean50 <- ames %>%
  ...
  summarize(samp_mean = mean(price))

sample_mean50 %>% summarise(mean_price = mean(samp_mean))

ggplot(sample_mean50, aes(x = samp_mean))+geom_histogram()
...
sample_mean150 %>% summarise(mean_price = mean(samp_mean))

ggplot(sample_mean150, aes(x = samp_mean))+geom_histogram()
...
sample_price15 <- ames %>% sample_n(size = 15) 

sample_price15 %>% summarise(mean_price = mean(price))

ggplot(sample_price15, aes(x = price))+geom_histogram()
...
samp_means15 <- ames %>%
  ...
  summarise(samp_mean_price = mean(price))
samp_means15 %>% summarise(mean_price = mean(samp_mean_price))

ggplot(samp_means15, aes(x = samp_mean_price))+geom_histogram()
```
...
ggplot(samp_means150, aes(x = samp_mean_price))+geom_histogram()
